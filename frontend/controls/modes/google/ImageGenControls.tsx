...
export const ImageGenControls: React.FC<ImageGenControlsProps> = (props) => {
  ...
  // 修 silent-failure-hunter MEDIUM-2：把 enableThinking 提到与其他 control 字段一致的
  // 局部变量分层，避免内联 `controls?.setEnableThinking(!enableThinking)` 在
  // controls=undefined 时静默 noop（click 无反馈）
  const enableThinking = controls?.enableThinking ?? false;
  const setEnableThinking = controls?.setEnableThinking ?? (() => {});
          ...
          <div className="mt-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    {outputMimeOptions.map((opt) => (
                      <button
                        ...
                        onClick={() =>
                          typeof opt.value === 'string' && setOutputMimeType(opt.value)
                        }
            ...
            <div className="flex items-center justify-between py-1">
              ...
              <div
                ...
                className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-200 ${
                  enableThinking ? 'bg-cyan-600' : 'bg-slate-600'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-200 ${
