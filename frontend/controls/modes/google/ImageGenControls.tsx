...
export const ImageGenControls: React.FC<ImageGenControlsProps> = (props) => {
  ...
  const seed = controls?.seed ?? propSeed ?? -1;
  ...
  const enhancePrompt = controls?.enhancePrompt ?? propEnhancePrompt ?? DEFAULT_CONTROLS.enhancePrompt;
  const setEnhancePrompt = controls?.setEnhancePrompt ?? propSetEnhancePrompt ?? (() => {});

  // Google 可用比例
  const availableRatios = useMemo(() => getAvailableAspectRatios('google'), []);

  // Gen 模式最大 2K
  const availableResolutionTiers = GOOGLE_GEN_RESOLUTION_TIERS;
