...
export const ImageGenControls: React.FC<ImageGenControlsProps> = (props) => {
  ...
  return (
    <GenericImageGenControls
      ...
      setOutputCompressionQuality={setOutputCompressionQuality}
...
const GenericImageGenControls: React.FC<GenericImageGenControlsProps> = ({
  ...
  enhancePrompt = DEFAULT_CONTROLS.enhancePrompt, setEnhancePrompt,
}) => {
  ...
  return (
    <>
      {/* AdvancedToggle: 仅非 Google 模式显示 */}
      {!isGoogle && <AdvancedToggle showAdvanced={showAdvanced} setShowAdvanced={setShowAdvanced} />}
        ...
        <div className="relative">
          ...
          {showStyleMenu && (
            <>
              ...
              <div className="absolute bottom-full left-0 mb-2 w-48 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-20 overflow-hidden animate-[fadeIn_0.1s_ease-out]">
      ...
      {/* Person Generation parameter removed - API uses default (allow_adult) */}

      {/* Google Imagen 工具栏按钮: Format */}
      {isGoogle && setOutputMimeType && (
        <div className="relative">
          <button onClick={() => setShowFormatMenu(!showFormatMenu)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-800/50 text-slate-300 hover:bg-slate-800 border border-transparent hover:border-slate-600 transition-all">
            <FileImage size={14} className="text-cyan-400" />
            {outputMimeType === 'image/png' ? 'PNG' : 'JPEG'}
          </button>
          {showFormatMenu && (
            <>
              <div className="fixed inset-0 z-10" onClick={() => setShowFormatMenu(false)} />
              <div className="absolute bottom-full left-0 mb-2 w-28 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-20 p-1 animate-[fadeIn_0.1s_ease-out]">
                {[{ value: 'image/png', label: 'PNG' }, { value: 'image/jpeg', label: 'JPEG' }].map((opt) => (
                  <button key={opt.value} onClick={() => { setOutputMimeType(opt.value); setShowFormatMenu(false); }} className={`w-full text-left px-2 py-1.5 rounded-lg text-xs ${outputMimeType === opt.value ? 'bg-cyan-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}>{opt.label}</button>
                ))}
              </div>
            </>
          )}
      ...
      {isGoogle && setSeed && (
        <div className="relative">
          <button onClick={() => setShowSeedMenu(!showSeedMenu)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-800/50 text-slate-300 hover:bg-slate-800 border border-transparent hover:border-slate-600 transition-all">
            <Hash size={14} className="text-indigo-400" />
            {seed === -1 || seed === undefined ? 'Random' : seed}
          </button>
          {showSeedMenu && (
            <>
              <div className="fixed inset-0 z-10" onClick={() => setShowSeedMenu(false)} />
              <div className="absolute bottom-full left-0 mb-2 w-36 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-20 p-3 animate-[fadeIn_0.1s_ease-out]">
                <label className="text-[10px] font-medium text-slate-400 uppercase mb-2 block">Seed (-1 = random)</label>
                <input type="number" value={seed ?? -1} onChange={(e) => setSeed(parseInt(e.target.value))} placeholder="-1" className="w-full px-2 py-1.5 text-xs bg-slate-800 border border-slate-600 rounded-lg text-slate-300 focus:outline-none focus:border-indigo-500" />
              </div>
            </>
          )}
        </div>
      ...
      {/* Google Imagen 工具栏按钮: JPEG Quality */}
      {isGoogle && outputMimeType === 'image/jpeg' && setOutputCompressionQuality && (
        <div className="relative">
          <button onClick={() => setShowQualityMenu(!showQualityMenu)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-800/50 text-slate-300 hover:bg-slate-800 border border-transparent hover:border-slate-600 transition-all">
            <Sparkles size={14} className="text-yellow-400" />
            Q{outputCompressionQuality}%
          </button>
          {showQualityMenu && (
            <>
              <div className="fixed inset-0 z-10" onClick={() => setShowQualityMenu(false)} />
              <div className="absolute bottom-full left-0 mb-2 w-40 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-20 p-3 animate-[fadeIn_0.1s_ease-out]">
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[10px] font-medium text-slate-400 uppercase">Quality</label>
                  <span className="text-xs text-slate-300 font-mono">{outputCompressionQuality}%</span>
                </div>
                <input type="range" min="1" max="100" step="5" value={outputCompressionQuality || 80} onChange={(e) => setOutputCompressionQuality(parseInt(e.target.value))} className="w-full h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
              </div>
            </>
          )}
        </div>
      ...
      {/* Google Imagen 工具栏按钮: Enhance */}
      {isGoogle && setEnhancePrompt && (
        <button onClick={() => setEnhancePrompt(!enhancePrompt)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${enhancePrompt ? 'bg-pink-600 text-white border-pink-500' : 'bg-slate-800/50 text-slate-300 border-transparent hover:bg-slate-800 hover:border-slate-600'}`} title="Let AI enhance your prompt">
          <Wand2 size={14} />
          Enhance
        </button>
      )}
