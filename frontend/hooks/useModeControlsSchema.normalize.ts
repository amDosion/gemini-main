/**
 * Pure normalization layer for mode-controls schema payloads.
 *
 * Extracted 1:1 from `useModeControlsSchema.ts` so the hook module stays focused on
 * fetching/caching/state while this module owns the (side-effect-free) shape coercion
 * of raw `/api/modes/.../controls` responses. The hook re-exports the public types and
 * `getPixelResolutionFromSchema` so existing import paths are unchanged.
 */
