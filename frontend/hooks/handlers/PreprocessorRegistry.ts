...
import { Preprocessor, ExecutionContext } from './types';
...
export class PreprocessorRegistry {
  ...
  register(preprocessor: Preprocessor): void {
    ...
    this.preprocessors.sort((a, b) => (a.priority || 999) - (b.priority || 999));
  }
...
export class GoogleFileUploadPreprocessor implements Preprocessor {
  ...
  async process(context: ExecutionContext): Promise<ExecutionContext> {
    ...
    const uploadedAttachments = await this.uploadFiles(context.attachments);
   ...
   */
  private async uploadFiles(attachments: Attachment[]): Promise<Attachment[]> {
    const results = await Promise.allSettled(attachments.map((att) => this.uploadFile(att)));

    // 成功的使用处理结果，失败的保留原始附件（后续流程降级到 Base64）
    return results.map((result, index) =>
      result.status === 'fulfilled' ? result.value : attachments[index]
    );
  }
