...
class ConfigurationService {
  ...
  public async getFullSettings(): Promise<FullSettings> {
    return db.request<FullSettings>('/settings/full', { cache: 'no-store' });
  }
  ...
  public async clearProviderModelCache(providerId: string): Promise<void> {
    if (!providerId) return;
    await db.request(`/models/${encodeURIComponent(providerId)}/cache`, {
      method: 'DELETE',
    });
  }
  ...
  public async getActiveConfig(): Promise<ActiveAppConfig> {
    const profileId = await db.getActiveProfileId();
