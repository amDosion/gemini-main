...
/**
 * @deprecated This is legacy code. The main image generation flow now uses UnifiedProviderClient.
 * 
 * **Current Status**: This file is kept for backward compatibility and specialized features:
 * - Virtual Try-On functionality (if implemented)
 * - Direct SDK usage for advanced features
 * - Legacy integrations that haven't been migrated yet
 * 
 * **For New Features**: Please use `UnifiedProviderClient` instead, which:
 * - Routes through backend API for better security (no API key exposure)
 * - Supports Vertex AI configuration
 * - Provides consistent error handling
 * - Maintains session-based authentication
 * 
 * **Migration Guide**: See `.kiro/specs/google-image-gen-flow-analysis/design.md`
 * 
 * **Main Flow**: 
 * ```
 * User → ImageGenHandler → llmService → UnifiedProviderClient → Backend API
 * ```
 * 
 * @see UnifiedProviderClient for the current implementation
 * @see .kiro/specs/google-image-gen-flow-analysis/ for architecture details
 */
...
export async function generateTextToImage(
    ...
    if (isImagen) {
        ...
        // Note: personGeneration parameter has been removed
        // API will use its default value (allow_adult), which allows
