...
export class LLMFactory {
  ...
  static async initialize(): Promise<void> {
    if (
      this.initialized &&
      cacheManager.get<AIProviderConfig[]>(CACHE_DOMAINS.PROVIDER_TEMPLATES)
    ) {
      return;
    }
    ...
    try {
      ...
      // 但必须留下日志，否则启动期模板加载失败将完全不可见。
      console.warn('[LLMFactory] Failed to preload provider templates:', error);
      this.initialized = true;
    }
  }
