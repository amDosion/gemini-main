...
const ALLOWED_MEDIA_RENDER_FILES = new Set([
  path.join('frontend', 'components', 'common', 'RetainedMedia.tsx'),
]);
...
describe('media render boundary guard', () => {
  it('keeps raw video and audio elements behind the shared retained media lifecycle', () => {
    ...
    const violations = componentFiles.flatMap((filePath) => {
      ...
      if (ALLOWED_MEDIA_RENDER_FILES.has(relativePath)) return [];
      ...
      return fs
        ...
        .flatMap((line, index) => {
          ...
          if (!sourceLine.includes('<video') && !sourceLine.includes('<audio')) return [];
