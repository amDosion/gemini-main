...
class ApiDB {
  ...
  async getSessionHistoryStates(sessionId: string): Promise<SessionHistoryState[]> {
    const res = await this.request<{ states?: SessionHistoryState[] }>(
      `/sessions/${sessionId}/history-states`
    );
  ...
  async getSessionHistoryPreference(sessionId: string): Promise<SessionHistoryPreference> {
    const res = await this.request<SessionHistoryPreference>(
      `/sessions/${sessionId}/history-preferences`
    );
    return {
  ...
  async browseStorage(
    ...
    return this.request<StorageBrowseResponse>(
      `/storage/browse/${encodeURIComponent(storageId)}?${params.toString()}`
    );
  }
  ...
  async getStorageFileMetadata(
    urls: string[],
    forceRefresh: boolean = false
  ): Promise<StorageFileMetadataItem[]> {
