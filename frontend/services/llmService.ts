...
export class LLMService {
  ...
  private get currentProvider() {
    if (!this.protocol || !this.providerId) {
      throw new Error(
        'Provider not configured. Please configure a provider in Settings → Profiles.'
      );
    }
  ...
  public async generateImage(
    ...
    this.debugLog(
      '[llmService.generateImage] Prompt:',
  ...
  public async outPaintImage(
    ...
    if (this.currentProvider.outPaintImage) {
      const result = await this.currentProvider.outPaintImage(
        referenceImage,
        mergedOptions,
        this.apiKey,
        this.baseUrl
    ...
    if (dashscopeKey) {
      ...
      if (tongyiProvider.outPaintImage) {
        ...
        const result = await tongyiProvider.outPaintImage(
          referenceImage,
          mergedOptions,
          dashscopeKey,
          dsUrl
