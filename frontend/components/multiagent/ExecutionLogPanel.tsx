...
export const ExecutionLogPanel: React.FC<ExecutionLogPanelProps> = ({
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700 shadow-2xl z-10">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-semibold text-slate-100">执行日志</h3>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors ${
              showFilter ? 'bg-teal-500/20 text-teal-300' : 'hover:bg-slate-800 text-slate-400'
            }`}
            aria-label="过滤日志"
          >
            <Filter size={16} />
          ...
          <button
            ...
            className="p-1.5 hover:bg-slate-800 rounded transition-colors text-slate-400 disabled:opacity-40"
            aria-label="导出日志"
      ...
      <div
        ref={logContainerRef}
        className="h-[300px] overflow-y-auto p-4 space-y-2 font-mono text-xs bg-slate-950"
      >
