...
export const AgentImageGenSection: React.FC<AgentImageGenSectionProps> = ({
  ...
  const tier = nodeData.agentResolutionTier || '1K';
  const ratio = nodeData.agentAspectRatio || '1:1';
  return (
        ...
        >
          {TIERS.map((t) => (
            <option key={t.v} value={t.v}>
              {t.l} ({getResolutionLabel(t.v, ratio)})
            </option>
          ))}
        </select>
      </div>
          ...
          <div className="flex gap-1">
            <input
              ...
              onChange={(e) => {
                ...
                updateNodeData({ agentSeed: Number.isNaN(parsed) ? -1 : parsed });
              }}
