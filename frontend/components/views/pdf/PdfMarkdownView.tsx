...
export const PdfMarkdownView: React.FC<PdfMarkdownViewProps> = ({ data }) => {
  ...
  const markdownContent =
    data.markdown && typeof data.markdown === 'string' ? data.markdown : generateMarkdown(data);
    ...
    <div className="text-slate-300 leading-relaxed markdown-body prose prose-invert max-w-none">
      <ReactMarkdown
        ...
        components={{
          h1: ({ node, ...props }) => (
            <h1
              className="text-2xl font-bold mb-4 text-white border-b border-slate-700 pb-2"
              {...props}
            />
          ),
          h2: ({ node, ...props }) => (
            <h2 className="text-xl font-bold mb-3 text-white mt-6" {...props} />
          ),
          h3: ({ node, ...props }) => (
            <h3 className="text-lg font-semibold mb-2 text-white mt-4" {...props} />
          ),
          ...
          code: ({ node, ...props }) => (
            <code
              className="bg-slate-800 px-1.5 py-0.5 rounded text-sm font-mono text-indigo-300"
              {...props}
            />
          ),
          pre: ({ node, ...props }) => (
            <pre
              className="bg-slate-900/50 p-4 rounded-lg overflow-x-auto my-4 text-sm font-mono border border-slate-800"
              {...props}
            />
          ),
          blockquote: ({ node, ...props }) => (
            <blockquote
              className="border-l-4 border-indigo-500 pl-4 py-1 my-4 text-slate-400 italic bg-slate-800/30 rounded-r"
              {...props}
            />
          ),
          a: ({ node, href, ...props }) => {
          ...
          th: ({ node, ...props }) => (
            <th
              className="px-3 py-2 text-left text-xs font-semibold text-slate-300 border border-slate-700"
              {...props}
            />
          ),
          td: ({ node, ...props }) => (
            <td
              className="px-3 py-2 text-sm text-slate-200 border border-slate-700/50"
              {...props}
            />
          ),
