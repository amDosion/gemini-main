...
import { Layers, AlertCircle, User, Bot, Sparkles, SlidersHorizontal, RotateCcw } from 'lucide-react';
...
const arePropsEqual = (prevProps: ImageBackgroundEditViewProps, nextProps: ImageBackgroundEditViewProps) => {
...
export const ImageBackgroundEditView = memo(({
    messages,
    setAppMode,
    onImageClick,
    loadingState,
                        ...
                        <div className={`p-3 rounded-2xl max-w-full text-sm shadow-sm ${msg.role === Role.USER
                            ...
                            {msg.attachments?.filter(att => att.url && att.url.length > 0).map((att, idx) => (
                                <div
                                    ...
                                    className={`relative group mt-1 rounded-lg overflow-hidden border cursor-pointer transition-all ${activeImageUrl === att.url ? 'ring-2 ring-blue-500 border-transparent' : 'border-slate-700 hover:border-slate-500'
                                        }`}
                                >
                                    <img src={att.url} className="w-full h-32 object-cover bg-slate-900" alt="thumbnail" />
                                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                                        {activeImageUrl === att.url && <div className="bg-blue-500 w-2 h-2 rounded-full absolute top-2 right-2 shadow-sm" />}
                                    </div>
                                </div>
                            ))}
                            {msg.isError && (
