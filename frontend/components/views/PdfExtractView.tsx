...
export const PdfExtractView: React.FC<PdfExtractViewProps> = ({
  ...
  useEffect(() => {
    ...
    // 直接使用 apiClient 获取模板
    apiClient
      .get<{ success: boolean; templates: PdfExtractionTemplate[] }>('/api/pdf/templates')
      .then((response) => {
  ...
  const historyBatches = useMemo(() => {
    return messages.filter((m) => m.role === Role.MODEL && (m.content || m.isError)).reverse();
  }, [messages]);
            ...
            {loadingState !== 'idle' ? (
              <div className="flex flex-col items-center justify-center min-h-[400px] gap-6">
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-xs font-mono text-indigo-400 font-bold">
                    PDF
                  </div>
                </div>
            ...
            {activeAttachments.length > 0 && (
              <div className="flex gap-2 flex-wrap">
                {activeAttachments.map((att, idx) => (
                  <div
                    key={idx}
                    className="relative group flex items-center gap-2 bg-slate-800 rounded-lg px-2 py-1"
                  >
