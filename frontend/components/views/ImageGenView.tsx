...
export const ImageGenView: React.FC<ImageGenViewProps> = ({
  ...
  useEffect(() => {
    if (loadingState === 'loading') {
        ...
        // Close mobile history if a new generation starts (user likely wants to see result)
  ...
  return (
    <div className="flex-1 flex h-full bg-slate-950 overflow-hidden relative" onClick={() => setContextMenu(null)}>
        
        {/* Mobile History Toggle Button */}
        <button 
            onClick={() => setIsMobileHistoryOpen(true)}
            className="md:hidden absolute top-4 left-4 z-20 p-2.5 bg-slate-900/80 text-slate-300 rounded-xl backdrop-blur-md border border-slate-700 shadow-lg hover:text-white transition-colors"
            title="Open History"
        >
            <History size={20} />
        </button>
        ...
        {/* LEFT SIDEBAR: History (Batches) - Responsive */}
        <div className={`
            w-72 flex flex-col flex-shrink-0 z-40 bg-slate-900 border-r border-slate-800
        ...
        `}>
            <div className="p-3 border-b border-slate-800 flex items-center justify-between bg-slate-900/50 h-[57px]">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button 
                        onClick={() => setIsMobileHistoryOpen(false)}
                        className="p-1.5 hover:bg-slate-800 rounded-lg text-slate-500 hover:text-white md:hidden transition-colors"
                    >
                        <X size={16} />
                    </button>
                </div>
        ...
        {/* RIGHT MAIN AREA */}
        <div className="flex-1 flex flex-col min-w-0 relative bg-[url('https://grainy-gradients.vercel.app/noise.svg')]">
            
            {/* Batch Indicator - Shifted on mobile to avoid overlap with menu toggle */}
            {displayImages.length > 1 && (
                <div className="absolute top-4 left-16 md:left-4 z-10 animate-[fadeIn_0.3s_ease-out]">
            ...
            {/* Input Area */}
            <div className="p-6 bg-gradient-to-t from-slate-950 via-slate-950 to-transparent z-20">
