...
import { 
  ...
  SlidersHorizontal, RotateCcw, Sparkles
} from 'lucide-react';
...
const extractHistoryPrompts = (msg: Message): { originalPrompt: string; enhancedPrompt: string } => {
...
export const ImageGenView: React.FC<ImageGenViewProps> = ({
    ...
    const getDisplayImageAttachments = useCallback((attachments?: Attachment[]) => {
        return (attachments || []).filter((attachment) => Boolean(getGeneratedAttachmentUrl(attachment)));
    }, [getGeneratedAttachmentUrl]);

    const displayImages = useMemo(() => {
        return getDisplayImageAttachments(activeBatchMessage?.attachments).filter(att => att.url && att.url.length > 0);
