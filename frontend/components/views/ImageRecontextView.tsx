...
import { Sparkles, AlertCircle, Layers, User, Bot, SlidersHorizontal, RotateCcw, Paperclip, X, Image as ImageIcon, Send } from 'lucide-react';
...
export const ImageRecontextView = memo(({
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            console.error('[ImageRecontextView] handleGenerate 处理附件失败:', error);
            ...
            <div className="w-72 flex-shrink-0 border-l border-slate-800 bg-slate-900/50 flex flex-col h-full overflow-hidden">
                ...
                <div className="border-t border-slate-800 p-3 space-y-2 bg-slate-900/80">
                    ...
                    <textarea
                        ...
                        placeholder={(activeAttachments.length === 0 && !activeImageUrl) ? "请先上传图片..." : "描述新的上下文环境..."}
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        >
                            {loadingState !== 'idle' ? (
                                ...
                                <>
                                    <Sparkles size={18} />
                                    {(activeAttachments.length === 0 && !activeImageUrl) ? '请先上传图片' : '重新上下文'}
                                </>
                            )}
                        </button>
                    </div>
