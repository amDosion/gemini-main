...
const KaraokeLyrics: React.FC<{ text: string; currentTime: number; duration: number }> = React.memo(
  ({ text, currentTime, duration }) => {
    ...
    const { currentLineIndex, currentWordIndex, lineProgress } = useMemo(() => {
      if (lines.length === 0)
        return { currentLineIndex: -1, currentWordIndex: -1, lineProgress: 0 };
      ...
      for (let i = 0; i < currentLine.words.length; i++) {
        if (
          currentTime >= currentLine.words[i].startTime &&
          currentTime <= currentLine.words[i].endTime
        ) {
          wordIdx = i;
          break;
        } else if (
          currentTime > currentLine.words[i].endTime &&
          i === currentLine.words.length - 1
        ) {
          wordIdx = i;
          break;
        }
          ...
          return (
            <div
              ...
              className={`text-xl leading-relaxed transition-all duration-300 ${
                isCurrent ? 'text-slate-100 font-medium' : 'text-slate-500 font-normal opacity-60'
              }`}
              style={{
...
// Isolated, time-dependent display. Subscribes to the audio element's
// timeupdate/loadedmetadata events itself so the high-frequency progress
// updates (~4/second) re-render only this subtree instead of the whole
// AudioGenView state tree.
const AudioProgressDisplay: React.FC<{
  audioRef: React.RefObject<HTMLAudioElement | null>;
  src: string | null;
  text: string;
  isPlaying: boolean;
}> = React.memo(({ audioRef, src, text, isPlaying }) => {
  const [currentTime, setCurrentTime] = useState(0);
  const [duration, setDuration] = useState(0);

  useEffect(() => {
    const audio = audioRef.current;
    // Reset progress whenever the active source changes.
    setCurrentTime(0);
    setDuration(0);
    if (!audio) return;
    ...
    // Initialize from the element if it is already loaded/playing.
    if (audio.duration) {
      setDuration(audio.duration);
    }
    ...
    <div className="mt-4 w-full max-w-2xl">
      <div className="bg-slate-800/60 backdrop-blur-sm rounded-xl p-8 border border-slate-700/50 min-h-[200px] flex items-center justify-center">
        <div className="w-full">
          ...
          <KaraokeLyrics text={text} currentTime={currentTime} duration={duration} />
        </div>
      </div>
      {duration > 0 && (
...
AudioProgressDisplay.displayName = 'AudioProgressDisplay';
...
export const AudioGenView: React.FC<AudioGenViewProps> = ({
  ...
  // Reset playback status when audio URL changes
  useEffect(() => {
    setIsPlaying(false);
  }, [activeAudioUrl]);
        ...
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              ...
              {/* Message Bubble */}
              <div
                className={`p-3 rounded-2xl max-w-full text-sm shadow-sm border ${
                  msg.role === Role.USER
            ...
            {loadingState !== 'idle' ? (
              <div className="flex flex-col items-center gap-6 p-8 rounded-3xl bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 shadow-2xl relative z-10">
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-sm font-mono text-cyan-400 font-bold tracking-widest">
                    TTS
                  </div>
                </div>
