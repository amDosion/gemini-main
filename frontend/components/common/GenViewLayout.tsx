...
export const GenViewLayout: React.FC<GenViewLayoutProps> = React.memo(
  ({
    ...
    const activeResizeHandlersRef = React.useRef<{
    ...
    const scheduleLeftSidebarWidthPaint = React.useCallback(
      (nextWidth: number) => {
      ...
      // applyPendingLeftSidebarWidth is a stable useCallback([]); listed to satisfy
      // exhaustive-deps, but this callback's identity never actually changes.
      [applyPendingLeftSidebarWidth]
    );
    ...
    return (
      <div className="flex-1 flex h-full bg-slate-950 overflow-hidden relative">
        ...
        >
          ...
          >
            <span
              className={`h-14 w-[2px] rounded-full transition-colors ${
          ...
          {/* Content */}
          <div className="flex-1 overflow-y-auto custom-scrollbar relative">{sidebar}</div>
        </div>

        {/* RIGHT MAIN AREA */}
        <div className="flex-1 flex flex-col min-w-0 bg-slate-950 relative">
          {/* Main Content (Canvas/Grid) */}
          <div className="flex-1 overflow-hidden relative flex flex-col">{main}</div>
          ...
          {bottom && (
            <div className="p-4 md:p-6 bg-gradient-to-t from-slate-950 via-slate-950 to-transparent z-20 shrink-0 border-t border-transparent md:border-none">
              <div className="mx-auto w-full">{bottom}</div>
            </div>
          )}
        </div>
      </div>
