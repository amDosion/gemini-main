...
interface ImageHistoryHoverPreview {
  messageId: string;
  role: Role;
  authorLabel: string;
  anchorX: number;
...
interface ImageHistoryHoverPreviewPosition {
  top: number;
  left: number;
  arrowOffsetY: number;
}

interface ImageHistoryHoverPreviewSize {
  width: number;
  height: number;
}

interface ImageHistoryActionMenuAnchor {
  messageId: string;
  anchorX: number;
  anchorY: number;
}

interface ImageHistoryActionMenuPosition {
  top: number;
  left: number;
}

type ImageHistoryAccent = 'orange' | 'pink' | 'emerald';

interface ImageHistorySidebarOptions {
  items: Message[];
...
const ACCENT_CLASSES: Record<ImageHistoryAccent, {
...
const USER_SELECTED_CLASS = 'ring-1 ring-blue-400/80 border-transparent bg-blue-500/10';
const USER_IDLE_CLASS = 'border-blue-500/20 bg-blue-500/5 hover:border-blue-400/40';

const getAttachmentPreviewGridClass = (count: number): string => {
...
const getAttachmentPreviewButtonClass = (count: number): string => (
  count <= 1
    ? 'h-28 p-2'
    : 'aspect-square p-1'
);

const getAttachmentPreviewImageClass = (count: number): string => (
  count <= 1
