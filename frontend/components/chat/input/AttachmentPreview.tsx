...
const getFileIcon = (att: Attachment) => {
  ...
  if (att.name === 'YouTube Video' || (att.fileUri && att.fileUri.includes('youtube'))) {
  ...
  return (
    <div className="flex gap-2 mb-2 overflow-x-auto p-1 custom-scrollbar justify-start">
      {attachments.map((att) => {
        ...
        const imageDisplayUrl = isImageAttachment ? getPreferredImageAttachmentUrl(att) : null;
        const matchedRole = roleOptions.find((option) => option.value === att.role)?.value;
        // 未设置 role 时显示 roleOptions[0]：提交路径会以同一默认值落库
        // （applyDefaultVideoAttachmentRoles）；已设置但不在选项内时如实显示原值，
        // 避免界面展示与实际提交的 role 不一致。
        const selectedRole = matchedRole ?? att.role ?? roleOptions[0]?.value ?? '';

        return (
          <div key={att.id} className="relative group shrink-0 w-20">
            <div
              className="w-16 h-16 rounded-lg border border-slate-700 bg-slate-800 overflow-hidden flex flex-col items-center justify-center p-1 relative"
              title={att.name}
            >
            ...
            {roleOptions.length > 0 && onRoleChange && (
              <label className="mt-1 block text-[10px] text-slate-500">
                ...
                >
                  {!matchedRole && att.role && <option value={att.role}>{att.role}</option>}
