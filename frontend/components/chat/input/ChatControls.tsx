...
export const ChatControls: React.FC<ChatControlsProps> = ({
  ...
  return (
    <div className="flex items-center gap-2 mr-2 border-r border-slate-700/50 pr-2 overflow-x-auto scrollbar-hide max-w-[200px] sm:max-w-none">
      <button 
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
          !canSearch ? 'bg-slate-800/20 text-slate-600 cursor-not-allowed opacity-50' 
      ...
      {canBrowse && (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
              enableBrowser ? 'bg-orange-600/20 text-orange-300 border-orange-500/50'
              : 'bg-slate-800/50 text-slate-400 border-transparent'
            }`}
      ...
      {canRAG && (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
              enableRAG ? 'bg-teal-600/20 text-teal-300 border-teal-500/50'
              : 'bg-slate-800/50 text-slate-400 border-transparent'
            }`}
      ...
      {canCache && (
          <button 
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
              googleCacheMode === 'exact' ? 'bg-blue-600/20 text-blue-300 border-blue-500/50' 
      ...
      <button 
        ...
        disabled={!canThink} 
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
          !canThink ? 'bg-slate-800/20 text-slate-600 cursor-not-allowed opacity-50' 
      ...
      <button 
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
          !canUrlContext ? 'bg-slate-800/20 text-slate-600 cursor-not-allowed opacity-50' 
      ...
      <button 
        ...
        disabled={!canCode} 
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border shrink-0 ${
          !canCode ? 'bg-slate-800/20 text-slate-600 cursor-not-allowed opacity-50' 
