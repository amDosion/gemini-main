...
export const EditorTab: React.FC<EditorTabProps> = ({
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const providerInstance = LLMFactory.getProvider(
        formData.protocol as ApiProtocol,
        formData.providerId
      );
              ...
              {isLoadingTemplates ? (
                ...
                providerTemplates.map((p) => (
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 md:p-2 rounded-lg border text-left transition-all ${
                  ...
                  >
                    <div
                      className={`w-2 h-2 md:w-1.5 md:h-1.5 rounded-full ${formData.providerId === p.id ? 'bg-indigo-500' : 'bg-slate-600'}`}
                    />
