...
export const OllamaModelManager: React.FC<OllamaModelManagerProps> = ({
  ...
  // 初始加载：ref-mirror loadModels 让 useEffect deps 仅响应 baseUrl/apiKey 变化，
  // 避免父 re-render 让 useCallback 重建触发重 fetch（修复同类 StrictMode 双 mount 问题）
      ...
      <div className="bg-slate-900/30 rounded-xl border border-slate-800/50 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-600">({models.length} models)</span>
          </div>
          {isExpanded ? (
              ...
              {isLoading && models.length === 0 ? (
                ...
                <div className="space-y-2">
                  {models.map((model) => (
                    <div
                      key={model.name}
                      className="bg-slate-900/50 rounded-lg border border-slate-800/50"
                    >
                              ...
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                <div>
                                  ...
                                  <span className="ml-1 text-slate-300">
                                    {modelInfo.details?.family || 'N/A'}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-slate-500">Parameters:</span>
                                  <span className="ml-1 text-slate-300">
                                    {modelInfo.details?.parameterSize || 'N/A'}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-slate-500">Format:</span>
                                  <span className="ml-1 text-slate-300">
                                    {modelInfo.details?.format || 'N/A'}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-slate-500">Quantization:</span>
                                  <span className="ml-1 text-slate-300">
                                    {modelInfo.details?.quantizationLevel || 'N/A'}
                                  </span>
                                </div>
                              </div>
                              <div>
