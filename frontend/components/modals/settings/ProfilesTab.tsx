    ...
    try {
      ...
      if (models.length > 0) {
        ...
        if (profile.cachedModelCount !== models.length) {
          ...
          onSaveProfile({ ...profile, cachedModelCount: models.length, savedModels: models }).catch(
            (e: unknown) => console.error('[ProfilesTab] Background cache-count update failed:', e)
          );
        }
      } else {
    ...
    return (
      <div className="absolute inset-0 bg-slate-950 z-10 flex flex-col animate-[fadeIn_0.2s_ease-out]">
        <div className="flex items-center gap-3 p-4 border-b border-slate-800 bg-slate-900/50">
          <button
            onClick={() => {
              // Cancel any in-flight model fetch so it no longer updates preview state
              abortControllerRef.current?.abort();
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3 mb-2">
                      <div
                        className={`p-2 rounded-lg shrink-0 ${isHidden ? 'bg-slate-800 text-slate-600' : 'bg-indigo-500/10 text-indigo-400'}`}
                      >
                    ...
                    {isHidden && (
                      <div className="mt-auto pt-2 border-t border-slate-800">
                        <span className="text-[10px] bg-slate-800 px-2 py-1 rounded text-slate-500 font-medium">
                          Hidden
                        </span>
                      </div>
                    )}
      ...
      <div className="shrink-0 flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          <h3 className="text-base md:text-lg font-medium text-white mb-0.5">
            Configuration Profiles ({safeProfiles.length})
          </h3>
          <p className="text-xs text-slate-500">
            Manage API setups. Active profile is used for chat.
          </p>
        </div>
        <button
          onClick={onCreateNew}
        ...
        {safeProfiles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {safeProfiles.map((p) => {
              ...
              if (p.providerId.includes('deepseek')) {
                activeBorder = 'border-blue-500/50';
                activeBg = 'bg-blue-600';
              } else if (p.providerId.includes('tongyi')) {
                activeBorder = 'border-purple-500/50';
                activeBg = 'bg-purple-600';
              } else if (p.providerId.includes('openai')) {
                activeBorder = 'border-emerald-500/50';
                activeBg = 'bg-emerald-600';
              } else if (p.providerId.includes('google')) {
              ...
              return (
                <div
                  key={p.id}
                  className={`group rounded-xl border p-4 md:p-5 h-full flex flex-col transition-all ${
                    isActive
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div
                        className={`p-2 rounded-lg shrink-0 ${isActive ? `${activeBg} text-white` : 'bg-slate-800 text-slate-500'}`}
                      >
