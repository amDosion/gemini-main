...
class ProductBackgroundEditingHandler:
    mode_id = "PRODUCT_BACKGROUND_EDITING"

    def execute(self, req: JobRequest, client, platform: str) -> JobResponse:
        if platform != "vertex":
            raise ValueError("PRODUCT_BACKGROUND_EDITING is Vertex-only")

        ssot = load_ssot()["modes"][self.mode_id]
        model = req.model or ssot["default_model"]

        if not req.image:
            raise ValueError("image is required for PRODUCT_BACKGROUND_EDITING")

        prompt = req.prompt or "Replace the background with a clean studio background."

        p = req.params or {}
        mask_dilation = float(p.get("mask_dilation", 0.0))
        ...
        resp = client.models.edit_image(
            ...
            reference_images=[
                ...
                MaskReferenceImage(
                    reference_id=2,
                    config=MaskReferenceConfig(mask_mode="MASK_MODE_BACKGROUND", mask_dilation=mask_dilation),
                ),
            ],
