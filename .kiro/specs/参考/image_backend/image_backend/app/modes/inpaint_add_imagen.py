...
class InpaintAddImagenHandler:
    mode_id = "INPAINT_ADD"

    def execute(self, req: JobRequest, client, platform: str) -> JobResponse:
        if platform != "vertex":
            raise ValueError("INPAINT_ADD is Vertex-only")

        ssot = load_ssot()["modes"][self.mode_id]
        model = req.model or ssot["default_model"]

        if not req.prompt:
            raise ValueError("prompt is required for INPAINT_ADD")
        if not req.image:
            raise ValueError("image is required for INPAINT_ADD")
        if not req.mask:
            raise ValueError("mask is required for INPAINT_ADD")
