...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  【Google Provider - 协调者模式】                                        │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │  google_service.py (GoogleService) - 统一入口/协调者              │  │
│  │  ├── ChatHandler          → 聊天处理                             │  │
│  │  ├── ImageGenerator       → 图片生成 (Imagen)                    │  │
│  │  ├── ImageEditCoordinator → 图片编辑协调                         │  │
│  │  ├── ExpandService        → 图片扩展                             │  │
│  │  ├── TryOnService         → 虚拟试穿                             │  │
│  │  ├── VideoService         → 视频生成 (Veo)                       │  │
│  │  ├── AudioService         → 音频生成                             │  │
│  │  └── CacheIntegration     → 缓存集成                             │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  【Tongyi Provider - 需要创建协调者】                                    │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │  tongyi_service.py (TongyiService) - 需要创建统一协调者           │  │
│  │  ├── 委托给 QwenNativeProvider    → 聊天服务                      │  │
│  │  ├── 委托给 TongyiImageService    → 图片生成                      │  │
│  │  ├── 委托给 ImageEditService      → 图片编辑                      │  │
│  │  ├── 委托给 ImageExpandService    → 图片扩展                      │  │
│  │  └── 委托给 TongyiVideoService    → 视频生成                      │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  【OpenAI Provider - 协调者模式】（需要重构为委托模式）                  │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │  openai_service.py (OpenAIService) - 协调者                       │  │
│  │  ├── ChatHandler          → 聊天处理（需要提取）                   │  │
│  │  ├── ImageGenerator       → 图片生成（需要提取）                   │  │
│  │  └── 其他子服务...                                                │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  【Ollama Provider - 协调者模式】（需要重构为委托模式）                  │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │  ollama_service.py (OllamaService) - 协调者                       │  │
│  │  ├── NativeOllamaHandler    → 原生 API 处理（需要提取）           │  │
│  │  └── OpenAICompatibleHandler → OpenAI 兼容处理（需要提取）         │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│  路由层（包含业务逻辑 + 直接调用服务）                          │
├─────────────────────────────────────────────────────────────┤
│  generate.py      → 直接调用各种服务                          │
│  image_edit.py    → 直接调用 tongyi.ImageEditService         │
│  image_expand.py  → 直接调用 tongyi.ImageExpandService       │
│  tongyi_chat.py   → 直接调用 tongyi.QwenNativeProvider       │
│  google_modes.py  → 调用 GoogleService（协调者）              │
│  qwen_modes.py    → 直接调用各 Tongyi 服务                    │
│  tryon.py         → 调用 GoogleService.try_on()              │
│  ...                                                         │
└─────────────────────────────────────────────────────────────┘
```
...
| 问题 | 影响 | 严重程度 |
|------|------|----------|
| 路由层包含业务逻辑 | 难以测试、难以复用 | 高 |
| 路由分散不统一 | 前端调用复杂、维护困难 | 高 |
| Provider 服务架构不一致 | 无法统一抽象 | 高 |
| Google 有协调者，其他 Provider 缺少统一协调者 | 分发逻辑复杂 | 高 |
| 认证逻辑重复 | 代码冗余、易出错 | 中（已解决） |
| 每个 provider 有专用路由 | 扩展困难 | 中 |
