...
class QwenNativeProvider(BaseProviderService):
    ...
    async def chat(self, messages: list, model: str, **kwargs) -> Dict[str, Any]:
        ...
        if is_vision_model(model):
            multimodal_messages = self._coerce_multimodal_messages(messages)
            response = await loop.run_in_executor(
                None,
                lambda: self._sync_multimodal_chat(messages=multimodal_messages, model=model, **kwargs)
            )
    ...
    async def stream_chat(self, messages: list, model: str, **kwargs):
        ...
        if is_vision_model(model):
            multimodal_messages = self._coerce_multimodal_messages(messages)
            def sync_stream():
                return self._sync_stream_multimodal_chat(messages=multimodal_messages, model=model, **kwargs)
