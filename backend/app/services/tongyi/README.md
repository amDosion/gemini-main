...
- Tongyi 的 Agent 执行入口在共享层：`backend/app/services/agent/`
- 共享层通过 `services/llm/adapters/tongyi_adapter.py` + `ProviderFactory.create("tongyi")` 调用到 `TongyiService`
- 因此 Tongyi 目录内不强制需要单独的 `agent/` 子目录
