...
class TongyiService(BaseProviderService):
    ...
    _CHAT_ROUTING_KWARGS = {"client_preference", "operation_type", "connection_mode"}
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.secondary_base_url = (
            kwargs.get("secondary_base_url")
            or ProviderConfig.get_secondary_base_url("tongyi")
            or self.api_url
        )

        # 子服务延迟加载（避免循环导入和减少初始化开销）
        self._chat_provider = None  # Primary (DashScope native)
        self._primary_connection_mode = None
        self._secondary_chat_provider = None  # Secondary (OpenAI-compatible)
    ...
    def _resolve_connection_mode(self, kwargs: Dict[str, Any]) -> str:
        ...
        if requested in {"official", "proxy"}:
            return requested
        return "official"
    ...
    def _get_primary_chat_provider(self, connection_mode: Optional[str] = None):
        ...
        if self._chat_provider is None or self._primary_connection_mode != target_mode:
            ...
            self._primary_connection_mode = target_mode
        return self._chat_provider

    def _get_secondary_chat_provider(self):
        if self._secondary_chat_provider is None:
            from ..openai.openai_service import OpenAIService
            secondary_url = self.secondary_base_url or self.api_url
            self._secondary_chat_provider = OpenAIService(
                api_key=self.api_key,
                api_url=secondary_url,
                timeout=self.kwargs.get("timeout", 120.0),
                max_retries=self.kwargs.get("max_retries", 3),
            )
        return self._secondary_chat_provider

    def _select_chat_client_order(self, model: str, **kwargs) -> Tuple[str, str]:
        preferred = str(kwargs.get("client_preference") or "").strip().lower()
        if preferred in {"primary", "secondary"}:
            first = preferred
        else:
            default_first = "primary" if self._resolve_connection_mode(kwargs) == "official" else "secondary"
            first = default_first

            if self.client_selector:
                try:
                    operation = {
                        "type": kwargs.get("operation_type", "chat"),
                        "provider_id": "tongyi",
                        "model": model,
                    ...
                    capabilities = ProviderCapabilities(
                        ...
                        web_search=True,
                        code_execution=True,
                        secondary_client_supported=True,
                    )
                    preferences = UserPreferences(preferred_client=None)
                    selected = self.client_selector.select(operation, capabilities, preferences)
                    if selected in {"primary", "secondary"}:
                        first = selected
                except Exception as exc:
                    logger.warning("[TongyiService] Client selector failed, using default client: %s", exc)

        second = "secondary" if first == "primary" else "primary"
        return first, second

    def _get_chat_client(self, client_type: str, connection_mode: str):
        if client_type == "secondary":
            return self._get_secondary_chat_provider()
        return self._get_primary_chat_provider(connection_mode=connection_mode)
    ...
    async def chat(
        ...
        **kwargs
        ...
        Returns:
            聊天响应字典
        """
        connection_mode = self._resolve_connection_mode(kwargs)
        request_kwargs = self._sanitize_chat_kwargs(kwargs)
        client_order = self._select_chat_client_order(model, operation_type="chat", **kwargs)
        last_error = None

        for idx, client_type in enumerate(client_order):
            client = self._get_chat_client(client_type, connection_mode)
            try:
                return await client.chat(messages, model, **request_kwargs)
            except Exception as exc:
                last_error = exc
                if idx == 0:
                    logger.warning(
                        "[TongyiService] Primary chat client '%s' failed, trying fallback '%s': %s",
                        client_type,
                        client_order[1],
                        exc,
                    )
        ...
        if last_error:
            raise last_error
        raise RuntimeError("Tongyi chat routing failed without explicit error")
    ...
    async def stream_chat(
        ...
        **kwargs
        ...
        for idx, client_type in enumerate(client_order):
            ...
            emitted_chunk = False
            try:
                async for chunk in client.stream_chat(messages, model, **request_kwargs):
                    emitted_chunk = True
                    yield chunk
                return
            except Exception as exc:
                last_error = exc
                # 仅在未产生任何 chunk 时允许切备，避免双流混杂导致结果不确定
                if idx == 0 and not emitted_chunk:
                    logger.warning(
                        "[TongyiService] Stream client '%s' failed before output, trying fallback '%s': %s",
                        client_type,
                        client_order[1],
                        exc,
                    )
        ...
        if last_error:
            raise last_error
        raise RuntimeError("Tongyi stream routing failed without explicit error")
    ...
    async def layered_design(
        ...
        **kwargs
        ...
        llm_client = self._get_primary_chat_provider(connection_mode=self.connection_mode)
