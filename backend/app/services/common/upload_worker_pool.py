...
class UploadWorkerPool:
    ...
    async def _process_task(self, task_id: str, worker_name: str):
        ...
        try:
            ...
            result = await StorageService.upload_file(
                ...
                content=content,
                content_type=self._resolve_task_content_type(db, task),
    ...
    async def _download_google_provider_asset_for_task(
        self,
        task: UploadTask,
        worker_name: str,
        asset_ref: str,
        ...
        try:
            ...
            binary, _ = await download_google_video_asset_for_user(
                lookup_db,
                resolved_user_id,
    ...
    def _resolve_task_content_type(self, db, task: UploadTask) -> str:
        from ...models.db_models import MessageAttachment
        ...
        if task.attachment_id:
            ...
            attachment_mime_type = str(getattr(attachment, "mime_type", "") or "").strip().lower()
        ...
        candidate_paths = [
            str(getattr(task, "filename", "") or "").strip(),
            str(getattr(task, "source_ai_url", "") or "").strip(),
            str(getattr(task, "source_url", "") or "").strip(),
            str(getattr(task, "source_file_path", "") or "").strip(),
        ]
        for candidate_path in candidate_paths:
            if not candidate_path:
                continue
            guessed_mime_type, _ = mimetypes.guess_type(candidate_path)
            normalized_guessed_mime_type = str(guessed_mime_type or "").strip().lower()
            if normalized_guessed_mime_type:
                return normalized_guessed_mime_type
    ...
    async def _update_session_attachment(
        ...
        if expected_user_id:
            attachment_query = attachment_query.filter(MessageAttachment.user_id == expected_user_id)
