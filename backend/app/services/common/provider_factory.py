...
class ProviderFactory:
    ...
    Caching Strategy:
    - Cache key includes provider, api_key, api_url, user_id and core init flags
    - Context-bound services (google/google-custom/tongyi/openai with db session) skip cache
      to avoid reusing stale request-scoped DB sessions
    ...
    def _build_cache_key(
        ...
        """
        Build a deterministic cache key for stateless service instances.

        Only includes initialization flags that may change service behavior.
        """
        relevant_keys = ("use_official_sdk", "use_vertex", "project", "location")
        config_fingerprint = "|".join(
            f"{k}={kwargs.get(k)!r}" for k in relevant_keys if k in kwargs
        )
        return (
            f"{provider}:{api_key}:"
            f"url={api_url or ''}:"
    ...
    def _is_cacheable_request(
        ...
        """
        Services bound to request DB/session must not be cached.
        """
        return not (provider in cls._db_context_providers and db is not None)
    ...
    def create(
        ...
        **kwargs
        ...
        Caching Logic:
        1. Build cache key from provider/api_key/api_url/user_id/init flags
        2. If cacheable and exists in cache: return cached instance
        3. Else: create new instance, optionally cache it, then return
        ...
        # Check cache first
        if should_use_cache and cache_key in cls._client_cache:
        ...
        try:
            ...
            # Tongyi/OpenAI: for request-scoped attachment/media workflows
            if provider in ['google', 'google-custom', 'tongyi', 'openai']:
                service = service_class(api_key, api_url, user_id=user_id, db=db, **kwargs)
            else:
                service = service_class(api_key, api_url, **kwargs)
            
            # Cache before returning (only for stateless requests)
            if should_use_cache:
                cls._client_cache[cache_key] = service
            else:
                logger.debug(
                    f"[ProviderFactory] Skip cache for context-bound request: "
    ...
    def get_cached(
        ...
        **kwargs
        ...
        # Backward-compatible fallback for legacy callers that only know provider+api_key.
        legacy_prefix = f"{provider}:{api_key}:"
        matches = [service for key, service in cls._client_cache.items() if key.startswith(legacy_prefix)]
        if len(matches) == 1:
            return matches[0]
        return None
