...
class InteractionsManager:
    ...
    async def create_interaction(
        ...
        # 集成 Memory Bank（如果启用）
        # 这里可以自动加载相关记忆并添加到 system_instruction
        memory_bank_enabled = agent_config and agent_config.get("memory_bank_id")
        ...
        try:
            ...
            interaction = client.interactions.create(
                ...
                agent_config=agent_config,  # 使用 agent_config 参数（不是 config）
    ...
    async def deep_research(
        ...
        **kwargs
        ...
        result = await self.create_interaction(
            ...
            vertexai=True,  # Deep Research 使用 Vertex AI
            user_id=user_id
        )
    ...
    async def get_interaction_status_async(
        ...
        if use_vertexai and (not project or not location) and self._db and user_id:
            ...
            db_project, db_location, _ = get_vertex_ai_credentials_from_db(
                user_id=user_id,
                db=self._db,
                project=project,
                location=location
            ...
            if hasattr(event, 'usage'):
                ...
                if usage:
                    event_dict['usage'] = {
                        ...
                        'prompt_tokens': getattr(usage, 'prompt_tokens', None) or getattr(usage, 'prompt_token_count', None),
                        'completion_tokens': getattr(usage, 'completion_tokens', None) or getattr(usage, 'candidates_token_count', None)
                    }
    ...
    async def stream_existing_interaction(
        ...
        try:
            logger.info(f"Streaming existing interaction: {interaction_id}, last_event_id={last_event_id}")
            ...
            def sync_stream_worker():
                ...
                try:
                    ...
                    for chunk in stream:
                        ...
                        # 处理不同类型的事件（与 InteractionsService 保持一致）
                        if chunk.event_type == "interaction.start":
                            ...
                            # 提取 grounding metadata（与 InteractionsService 保持一致）
                            try:
                                ...
                                logger.debug(f"[InteractionsManager] Could not extract grounding metadata: {e}")
