...
class ToolOrchestrator:
    ...
    def register_mcp_server(
        ...
        # 注意：新的 MCP 实现使用配置对象而不是 URL
        # 这里保留旧接口以保持兼容性，但实际需要更新为新接口
        # TODO: 更新为使用新的 MCP 配置系统
        logger.warning(f"MCP server registration with URL is deprecated. Use MCPManager instead.")
        logger.info(f"MCP server registration skipped: {server_name} at {server_url} (use MCPManager for new implementation)")
