...
def filter_supported_schema(schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for field_name, field_value in schema.items():
        # 跳过不支持的字段
        if field_name not in SUPPORTED_SCHEMA_FIELDS:
            logger.debug(f"Filtering out unsupported field: {field_name}")
            continue

        # 递归处理单个 Schema 字段（如 items）
        if field_name in SINGLE_SCHEMA_FIELDS:
            if isinstance(field_value, dict):
                filtered[field_name] = filter_supported_schema(field_value)
            else:
                filtered[field_name] = field_value

        # 递归处理 Schema 列表字段（如 any_of）
        elif field_name in LIST_SCHEMA_FIELDS:
        ...
        elif field_name in DICT_SCHEMA_FIELDS:
            if isinstance(field_value, dict):
                filtered[field_name] = {
