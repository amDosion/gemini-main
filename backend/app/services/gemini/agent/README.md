...
说明：
- 本目录导出的 `Orchestrator` / `CoordinatorAgent` / `SequentialAgent` / `ParallelAgent` 仍属于 legacy Google runtime 兼容能力。
- provider-neutral 的 Multi-Agent 主入口统一使用 `POST /api/modes/{provider}/multi-agent`。
...
兼容 helper 适用范围：
- 仅当你需要维护既有 Google/Gemini runtime orchestration 代码时，才直接导入 `app.services.gemini.agent` 中的这些 helper。
- 新功能不要把 `app.services.gemini.agent` 当成跨 provider 多智能体 SDK 使用。
...
说明：
- 该实现是 Google/Gemini runtime 侧的 orchestration helper，不是 provider-neutral 编排器。
- 面向新功能接入时，统一入口仍是 `POST /api/modes/{provider}/multi-agent`。
...
说明：
- 当前实现依赖 `GoogleService`，属于 legacy Google orchestration 兼容能力。
- 不应用作 OpenAI / 其他 provider 的统一多智能体入口。
...
说明：
- 当前实现属于 legacy Google runtime 顺序编排兼容能力。
- 不应用作 provider-neutral 的统一多智能体执行入口。
...
说明：
- 当前实现属于 legacy Google runtime 并行编排兼容能力。
- 不应用作 provider-neutral 的统一多智能体执行入口。
...
注意：
- 该入口已废弃，仅用于兼容历史 Google/Gemini orchestration 调用。
- 新接入应改用统一 provider mode：`POST /api/modes/{provider}/multi-agent`。
- 兼容入口成功响应会附带 `routeMeta.entrypoint=legacy-orchestrate`，并返回废弃提示 header。
...
POST /api/modes/{provider}/multi-agent
```
该入口走统一 provider mode，适合作为 OpenAI / Google / 其他 provider 的收敛接入面。

### Runtime endpoints

当前 runtime/session 主路径：

- `POST /api/multi-agent/agents/{agent_id}/runtime/run`
- `POST /api/multi-agent/agents/{agent_id}/runtime/run-live`
- `GET /api/multi-agent/agents/{agent_id}/runtime/sessions`
- `GET /api/multi-agent/agents/{agent_id}/runtime/sessions/{session_id}`
- `POST /api/multi-agent/agents/{agent_id}/runtime/sessions/{session_id}/confirm-tool`
- `POST /api/multi-agent/agents/{agent_id}/runtime/sessions/{session_id}/rewind`
- `POST /api/multi-agent/agents/{agent_id}/runtime/sessions/{session_id}/memory/index`
- `POST /api/multi-agent/agents/{agent_id}/runtime/memory/search`

兼容旧路径：

- 仍接受 `/api/multi-agent/adk/...`
- 旧路径仅用于兼容历史调用，OpenAPI 已标记为 deprecated
- 新接入统一使用 `/api/multi-agent/agents/{agent_id}/runtime/...`
...
```json
{
  ...
  "routeMeta": {
    ...
    "status": "deprecated",
    "runtimeKind": "google-adk",
    "providerScope": "google",
    "officialOrchestration": true,
    "supportsProviderSwitch": false,
    "recommendedEntrypoint": "provider-mode",
    "recommendedPathTemplate": "/api/modes/{provider}/multi-agent"
  }
}
```
