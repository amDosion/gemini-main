...
async def read_excel_file(
    ...
    try:
        logger.info(f"[ExcelTools] Reading Excel file: {file_path}")
...
async def clean_dataframe(
    ...
    try:
        ...
        # 从字典重建 DataFrame
        if "sample_data" in df_data:
            # 如果只有样本数据，需要完整数据
            logger.warning("[ExcelTools] Only sample data provided, cleaning may be incomplete")
            df = pd.DataFrame(df_data["sample_data"])
        else:
            # 假设 df_data 包含完整数据
...
async def analyze_dataframe(
    ...
    try:
        logger.info(f"[ExcelTools] Analyzing dataframe: {analysis_type}")
        
        # 从字典重建 DataFrame
        if isinstance(df_data, dict) and "cleaned_data" in df_data:
            df = pd.DataFrame(df_data["cleaned_data"])
        elif isinstance(df_data, dict) and "sample_data" in df_data:
            df = pd.DataFrame(df_data["sample_data"])
        else:
            df = pd.DataFrame(df_data)
        ...
        # 相关性分析
        if analysis_type in ["comprehensive", "correlation"] and len(numeric_cols) > 1:
            result["correlations"] = df[numeric_cols].corr().to_dict()
        
        # 分布分析
        if analysis_type in ["comprehensive", "distribution"] and numeric_cols:
        ...
        # 趋势分析（简化实现）
        if analysis_type in ["comprehensive", "trends"] and numeric_cols:
            result["trends"] = {}
            for col in numeric_cols:
                if len(df) > 1:
                    # 简单的线性趋势
