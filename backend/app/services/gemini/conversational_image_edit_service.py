...
class ConversationalImageEditService:
    ...
    async def send_edit_message(
        ...
        if should_include_image and reference_images:
            logger.debug(f"[ConversationalImageEdit] First round: including reference images")
        ...
        try:
            ...
            logger.error(f"[ConversationalImageEdit] Failed to send edit message: {e}", exc_info=True)
            raise
