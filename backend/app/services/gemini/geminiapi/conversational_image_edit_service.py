...
class ConversationalImageEditService:
    ...
    async def _send_edit_message_internal(
        ...
        if not chat:
            ...
            if chat_session.config_json:
                try:
                    ...
                    if genai_types and valid_config:
                        ...
                        logger.info(f"[DEBUG] chat_config 创建成功 (genai_types 路径): response_modalities=[TEXT, IMAGE], image_config={image_config_dict}, enable_thinking={enable_thinking}")
                    elif valid_config:
                        chat_config = {
                            'response_modalities': ['TEXT', 'IMAGE'],
                            'image_config': {},
                        ...
                        logger.info(f"[DEBUG] chat_config 创建成功 (字典路径): response_modalities={chat_config['response_modalities']}, image_config={chat_config['image_config']}, enable_thinking={enable_thinking}")
                except Exception as e:
            ...
            if chat_config is None:
                if genai_types:
                    ...
                    logger.info(f"[DEBUG] chat_config 创建成功 (兜底逻辑 genai_types): response_modalities=[TEXT, IMAGE], enable_thinking={enable_thinking}")
                else:
                    chat_config = {
                        'response_modalities': ['TEXT', 'IMAGE'],
                        # ✅ 禁用自动函数调用，避免 MALFORMED_FUNCTION_CALL 错误
                    ...
                    if enable_thinking and self._supports_thinking(chat_session.model_name):
                        chat_config['thinking_config'] = {'include_thoughts': True}
                    logger.info(f"[DEBUG] chat_config 创建成功 (兜底逻辑 字典): response_modalities={chat_config['response_modalities']}, enable_thinking={enable_thinking}")
        ...
        logger.info(f"[DEBUG] message_parts 数量: {len(message_parts)}")
        for i, part in enumerate(message_parts):
            part_type = type(part).__name__
            if hasattr(part, 'inline_data') and part.inline_data:
                logger.info(f"[DEBUG] message_parts[{i}]: type={part_type}, has_inline_data=True, mime_type={getattr(part.inline_data, 'mime_type', 'unknown')}")
            elif hasattr(part, 'text') and part.text:
                logger.info(f"[DEBUG] message_parts[{i}]: type={part_type}, has_text=True, text_preview={part.text[:50]}...")
            elif hasattr(part, 'file_data') and part.file_data:
                logger.info(f"[DEBUG] message_parts[{i}]: type={part_type}, has_file_data=True")
            else:
                logger.info(f"[DEBUG] message_parts[{i}]: type={part_type}, attrs={dir(part)[:5]}...")
        ...
        if len(message_parts) == 1 and genai_types:
            ...
            logger.info(f"[DEBUG] message_to_send: 使用纯文本字符串")
        elif len(message_parts) == 1:
            # 回退到 part 对象
            message_to_send = message_parts[0]
            logger.info(f"[DEBUG] message_to_send: 使用单个 part 对象")
        else:
            # 多个 parts（图片 + 文本），使用列表
            message_to_send = message_parts
            logger.info(f"[DEBUG] message_to_send: 使用 {len(message_parts)} 个 parts 的列表")
        ...
        try:
            ...
            if genai_types:
                ...
                logger.info(f"[DEBUG] send_config 创建成功: response_modalities=[TEXT, IMAGE], image_config={image_config_dict if image_config_dict else 'None'}, safety_settings=已禁用")

            # 调用 chat.send_message()（非流式）
            # 注意：Chat 对象会自动维护历史记录，所以直接调用即可
            # 如果提供了配置覆盖，使用它；否则使用创建 chat 时的默认配置
            logger.info(f"[DEBUG] 发送消息: send_config={'存在' if send_config else '不存在（使用 chat 默认配置）'}")
            logger.info(f"[DEBUG] 开始调用 chat.send_message()...")
            ...
            try:
                ...
                logger.info(f"[DEBUG] chat.send_message() 完成，耗时: {_elapsed:.2f}秒")
            except Exception as e:
                _elapsed = _time.time() - _start_time
            ...
            if content_parts:
                logger.info(f"[DEBUG] 响应包含 {len(content_parts)} 个 parts")
                for idx, part in enumerate(content_parts):
                    ...
                    logger.info(f"[DEBUG] Part {idx} 结构分析: thought={thought_value}, has_inline_data={inline_data_value is not None}, has_text={text_value is not None}")
            ...
            def process_single_image(img_item) -> Optional[Dict[str, Any]]:
                ...
                if isinstance(img_item, str):
                    ...
                    if img_item.startswith('http://') or img_item.startswith('https://'):
                        return {'url': img_item, 'mime_type': 'image/png'}
                    elif img_item.startswith('data:'):
                        return {'url': img_item, 'mime_type': 'image/png'}
                    else:
                        # 纯 base64：添加 data URL 前缀
                        return {'url': f"data:image/png;base64,{img_item}", 'mime_type': 'image/png'}
