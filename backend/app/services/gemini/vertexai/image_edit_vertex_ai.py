...
# Default model for image editing (used when no model is specified)
...
class VertexAIImageEditor(BaseImageEditor):
    ...
    def edit_image(
        ...
        # Get model from config (use as-is, no mapping)
        model = (config or {}).get('model', DEFAULT_EDIT_MODEL)
        logger.info(f"[VertexAIImageEditor] Using model: {model}")
        ...
        try:
            ...
            logger.error(f"[VertexAIImageEditor] Editing failed: {error_msg}")
