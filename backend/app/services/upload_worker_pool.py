    ...
    async def _update_session_attachment(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not session:
                    log_print(f"[{worker_name}] ⚠️ 会话不存在: {session_id[:8]}...", "WARNING")
                    return
                ...
                if updated:
                    ...
                    db.commit()
                    log_print(f"[{worker_name}] ✅ 会话附件 URL 已更新")
                    return

                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                else:
                    log_print(f"[{worker_name}] ⚠️ 重试后仍未找到附件: {attachment_id[:8]}...", "WARNING")
            except Exception as e:
                log_print(f"[{worker_name}] ❌ 更新会话失败: {e}", "ERROR")
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
