...
try:
    from ..services.dashscope_file_upload import upload_bytes_to_dashscope, upload_to_dashscope
except ImportError:
    from services.dashscope_file_upload import upload_bytes_to_dashscope, upload_to_dashscope
...
async def edit_image(request: ImageEditRequest) -> ImageEditResponse:
    ...
    """
    logger.info(f"[Image Edit] 收到图像编辑请求: model={request.model}")
