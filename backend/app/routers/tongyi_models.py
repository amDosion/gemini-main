...
def convert_to_model_configs(model_ids: List[str]) -> List[ModelConfig]:
    """
    转换模型配置
    
    将模型 ID 列表转换为 ModelConfig 格式
    """
    models = []
...
async def get_tongyi_models(
    ...
    try:
        ...
        logger.info(f"[Tongyi Models] Got {len(model_ids)} models from API")
        
        # 3. 转换为 ModelConfig 格式
