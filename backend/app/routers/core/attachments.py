...
def _resolve_attachment_provider_asset_ref(
    attachment: MessageAttachment,
) -> tuple[Optional[str], Optional[str], Optional[str]]:
    provider_file_name = normalize_gemini_file_name(
        getattr(attachment, "google_file_uri", None)
        or getattr(attachment, "file_uri", None)
        or getattr(attachment, "temp_url", None)
    )
    provider_file_uri = str(
        getattr(attachment, "file_uri", None)
        or getattr(attachment, "google_file_uri", None)
        or getattr(attachment, "temp_url", None)
        or ""
    ).strip() or None
    gcs_uri = provider_file_uri if provider_file_uri and provider_file_uri.startswith("gs://") else None
    return provider_file_name, provider_file_uri, gcs_uri


async def _proxy_google_provider_video(
    attachment: MessageAttachment,
    *,
    db: Session,
    user_id: str,
    ...
    binary, resolved_mime_type = await download_google_video_asset_for_user(
        db=db,
        user_id=user_id,
        provider_file_name=provider_file_name,
        provider_file_uri=provider_file_uri,
        gcs_uri=gcs_uri,
        mime_type=fallback_mime_type or getattr(attachment, "mime_type", None),
    )
    return Response(
        content=binary,
        media_type=resolved_mime_type or fallback_mime_type or "video/mp4",
        headers={
            'Cache-Control': 'no-cache, no-store, must-revalidate',
            'Pragma': 'no-cache',
            'Expires': '0'
...
async def get_temp_image(
    ...
    if not attachment.temp_url:
        ...
        if provider_file_name or gcs_uri:
            logger.info(f"[TempImage] Proxying Google provider asset: {attachment_id}")
            try:
                return await _proxy_google_provider_video(
                    attachment,
                    db=db,
    ...
    elif temp_url.startswith('files/') or temp_url.startswith('gs://') or normalize_gemini_file_name(temp_url):
        provider_file_name, provider_file_uri, gcs_uri = _resolve_attachment_provider_asset_ref(attachment)
        if temp_url.startswith("gs://"):
            gcs_uri = temp_url
        elif normalize_gemini_file_name(temp_url):
            provider_file_name = normalize_gemini_file_name(temp_url)
            provider_file_uri = temp_url

        logger.info(f"[TempImage] Proxying provider temp asset: {attachment_id}")
        try:
            return await _proxy_google_provider_video(
                attachment,
                db=db,
