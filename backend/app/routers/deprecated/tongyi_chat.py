...
def _decrypt_api_key(api_key: str) -> str:
    ...
    try:
        ...
        logger.warning(f"[Tongyi Chat] API key decryption failed: {e}")
        return api_key
...
async def get_tongyi_api_key(
    ...
    if request_api_key and request_api_key.strip():
        logger.info(f"[Tongyi Chat] Using API key from request parameter (test/override mode)")
    ...
    if active_profile_id:
        for profile in matching_profiles:
            if profile.id == active_profile_id and profile.api_key:
                logger.info(f"[Tongyi Chat] Using API key from active profile '{profile.name}'")
                return _decrypt_api_key(profile.api_key)

    # 回退：使用第一个匹配的配置
    for profile in matching_profiles:
        if profile.api_key:
            logger.info(f"[Tongyi Chat] Using API key from profile '{profile.name}' (fallback)")
            return _decrypt_api_key(profile.api_key)
...
class ChatRequest(BaseModel):
    ...
    options: ChatOptions
    apiKey: Optional[str] = None  # ✅ 可选，会从数据库获取
...
async def chat_tongyi(
    ...
    Args:
        request: 聊天请求
        request_obj: FastAPI Request 对象（用于认证）
        db: 数据库会话
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                from backend.app.services.tongyi.chat import QwenNativeProvider
