...
from ...core.dependencies import get_rate_limiter, get_research_cache, get_validator
...
async def start_research(
    ...
    try:
        ...
        try:
            ...
            # 注意：research.py 使用 authorization header，需要从数据库获取 Vertex AI 配置
            db = next(get_db())
            manager = get_interactions_manager(db=db, default_vertexai=True)
            
            # 提取 user_id（用于从数据库获取 Vertex AI 配置）
            user_id = extract_user_id(api_key)
            ...
            # 调用 InteractionsManager.create_interaction()（使用 Vertex AI，从数据库获取配置）
            ...
            # 从返回的字典获取 interaction 信息
            interaction_id = result.get('id')
            interaction_status = result.get('status')
            ...
            class Interaction:
                def __init__(self, id, status, outputs=None, error=None):
                    self.id = id
                    self.status = status
...
async def get_research_status(
    ...
    try:
        ...
        # 调用 InteractionsManager.get_interaction_status_async()
...
async def cancel_research(
    ...
    try:
        ...
        # 调用 InteractionsManager.delete_interaction()
...
async def followup_research(
    ...
    try:
        ...
        try:
            # 使用 Model 进行追问，速度更快
            # 注意：InteractionsManager 主要支持 agent，model 参数需要通过其他方式处理
            # 这里暂时使用 agent，后续可以扩展支持 model 参数
            result = await manager.create_interaction(
                input=request.question,
                api_key=api_key,
                agent="gemini-2.5-flash",  # 使用快速模型（作为 agent）
