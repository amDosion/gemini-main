...
def camel_to_snake(name: str) -> str:
    ...
    # 保留下划线前缀（例如 _templateMeta / __internalField）
    leading_underscores_match = re.match(r'^_+', name)
    leading_underscores = leading_underscores_match.group(0) if leading_underscores_match else ""
    core_name = name[len(leading_underscores):]
    if not core_name:
        return name

    # 处理连续大写字母（如 XMLParser -> xml_parser）
    s1 = re.sub('(.)([A-Z][a-z]+)', r'\1_\2', core_name)
    converted_core = re.sub('([a-z0-9])([A-Z])', r'\1_\2', s1).lower()
    return f"{leading_underscores}{converted_core}"
...
def snake_to_camel(name: str) -> str:
    ...
    # 保留下划线前缀（例如 _template_meta -> _templateMeta）
    leading_underscores_match = re.match(r'^_+', name)
    leading_underscores = leading_underscores_match.group(0) if leading_underscores_match else ""
    core_name = name[len(leading_underscores):]
    if not core_name:
    ...
    components = core_name.split('_')
    converted_core = components[0] + ''.join(x.title() for x in components[1:])
    return f"{leading_underscores}{converted_core}"
