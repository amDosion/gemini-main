...
class ResearchCache:
    ...
    A single lock guards both ``interactions`` and ``results`` so that cleanup,
    writes, and eviction within each public method form an atomic unit.  This
    protects against concurrent access when the singleton is shared across
    multiple threads or worker contexts.
    """
    ...
    def _cleanup_expired(self) -> None:
        """清理过期条目 (must be called with self._lock held)."""
        now = time.time()
    ...
    def _evict_oldest(self, cache: Dict[str, Any], max_size: int) -> None:
        """超出上限时移除最早过期的条目 (must be called with self._lock held)."""
    ...
    def delete_cached_interaction(self, interaction_id: str) -> None:
